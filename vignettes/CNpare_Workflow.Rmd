...
vignette: >
  %\VignetteIndexEntry{Indentifying the most representative CIN model}
...
## DATA DIRECTORY
# The file `component_parameters.rds` needs to be downloaded from repo (https://bitbucket.org/britroc/cnsignatures/src/master/data/) and then added to the `data/` folder for being able to run the vignette (this file has not been added to this repo due to size limitations)
...
-   `cells_segcn.RData` -- Absolute copy-number profiles of cancer cell lines generated with *ASCAT.sc*. The raw file can be found in the following github repository: `VanLoo-lab/ASCAT.sc`. The segment tables should have the following column headers: "chromosome", "start", "end", "segVal", "sample".
